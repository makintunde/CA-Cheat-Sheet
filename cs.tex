...
\textbf{Definition:}
A branch cut is a curve (with ends possibly open, closed, or
half-open) in the complex plane across which a holomorphic multivalued
function is \textit{discontinuous}.

\textbf{Example:} The holomorphic function $\text{Log } z$ has its branch cut defined by the
negative semi-axis $(- \infty, 0]$.
\subsection{Integration along curves}
For integration of a continuous function over a curve, we have:
\textit{ML Inequality:} $|\int_{\gamma}f(z) \,\text{d}z| \leq \sup_{z \in \gamma} |f(z)| \cdot \text{length} (\gamma).$

\textbf{Definition:} A primitive for $f$ on $\Omega \subset \mathbb{C}$ is a function $F$ that is holomorphic
